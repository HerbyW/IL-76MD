...
#
###################################################################################################
setprop("/sim/model/il76md/scalpel", "off");

setlistener("/sim/model/livery/file", func
{
  if ((getprop("/sim/model/livery/file") == "Scalpel-MD") and (getprop("/engines/engine/out-of-fuel") == 1))
      setprop("/sim/model/il76md/scalpel", "on");
    else
      setprop("/sim/model/il76md/scalpel", "off");
}
);
...
setlistener("/sim/model/il76md/scalpel", func
{
  if (getprop("/sim/model/il76md/scalpel") == "on")
      interpolate("/sim/model/il76md/tents", 100.0, 10, 100.0, 1);
    else
      interpolate("/sim/model/il76md/tents", 0.0, 10, 0.0, 1);
}, 1, 0);
...
#
#Rosenbauer Trucks roll out
#
setlistener("/controls/rosen/jump-signal", func(v) {
 if(v.getValue()){
    interpolate("/controls/rosen/jump-signal-pos", 1, 0.25);
    
  }else{
    interpolate("/controls/rosen/jump-signal-pos", 0, 0.25);
  }
});

#
#Rosenbauer Trucks loading
#
setlistener("/controls/rosen/load-signal", func(v) {
  if(v.getValue()){
    interpolate("/sim/weight[1]/weight-lb", 105000, 40);
  }else{
    interpolate("/sim/weight[1]/weight-lb", 0, 40);
  }
});
...
setprop("/sim/messages/copilot", "Have fun with the IL-76MD");
